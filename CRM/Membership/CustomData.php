...
class CRM_Membership_CustomData {
  public const CUSTOM_DATA_HELPER_VERSION = '0.8';
  ...
  public function syncCustomGroup($source_file) {
    ...
    foreach ($data['_fields'] as $customFieldSpec) {
      ...
      $has_option_group = isset($customFieldSpec['option_group_id']) && $customFieldSpec['option_group_id'] !== ''
        && $customFieldSpec['option_group_id'] !== '0';
      if ($has_option_group && !is_numeric($customFieldSpec['option_group_id'])) {
  ...
  protected function getEntityID($entity_type, $selector) {
    ...
    switch ($lookup_result['count']) {
      ...
      default:
        ...
        $this->log(
        self::CUSTOM_DATA_HELPER_LOG_ERROR,
        "Bad {$entity_type} lookup selector: " . json_encode($selector)
        );
        return 'FAILED';
      case 0:
        // not found
  ...
  protected function updateEntity(
    ...
    foreach ($requested_data as $field => $value) {
      ...
      // compare as strings, since the API may return numeric values as strings
    ...
    if ($force || $update_query !== []) {
      ...
      foreach ($required_fields as $required_field) {
        ...
        // else: neither requested nor current data has this field, nothing we can do
      }
    ...
    foreach (array_keys($data) as $key) {
      if (preg_match('/^(?P<group_name>\w+)[.](?P<field_name>\w+)$/', $key, $match)) {
        if ($customgroups === NULL || $customgroups === []
          ...
          // TODO: otherwise this is an unknown data field, and we currently silently ignore it
        }
      }
    }
  }
  ...
  public static function unREST(&$params, $group_names = NULL) {
    // also fall back to the default group list if an empty array was passed
    if (!is_array($group_names) || $group_names === []) {
  ...
  public static function getOptionValue(
    ...
    if ($label === NULL || $label === '' || $label === '0'
      || $group_name === NULL || $group_name === '' || $group_name === '0') {
      return NULL;
    }
