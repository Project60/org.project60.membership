...
class CRM_Membership_Form_Report_OutstandingMembershipFees extends CRM_Report_Form {
   ...
   */
  public function getCheckPeriodFrom() {
   ...
   */
  public function getSuitableMembershipFeeCustomFields() {
    $suitable_fields = [];
    $gsearch = civicrm_api3('CustomGroup', 'get', ['extends' => 'Membership']);
    if ($gsearch['count'] > 0) {
      foreach ($gsearch['values'] as $potential_group_id => $potential_group) {
        $fsearch = civicrm_api3('CustomField', 'get',
          ['data_type' => 'Money', 'custom_group_id' => $potential_group_id]);
         ...
         FROM  civicrm_membership {$this->_aliases['civicrm_membership']} {$this->_aclFrom}
               INNER JOIN civicrm_contact {$this->_aliases['civicrm_contact']}
                          ON {$this->_aliases['civicrm_contact']}.id =
                             ...
                             AND {$this->_aliases['civicrm_membership']}.is_test = 0
  ...
  public function alterDisplay(&$rows) {
    ...
    foreach ($rows as $rowNum => $row) {
      ...
      $rows[$rowNum]['civicrm_membership_membership_dues'] = sprintf('%01.2f',
        ($rows[$rowNum]['civicrm_membership_membership_dues'] / $period_factor)
      ...
      if (array_key_exists('civicrm_membership_membership_type_id', $row)) {
        if ($value = $row['civicrm_membership_membership_type_id']) {
          ...
          $url = CRM_Utils_System::url('civicrm/contact/view/membership',
            "action=view&reset=1&cid={$row['civicrm_contact_id']}&id={$row['civicrm_membership_membership_id']}");
