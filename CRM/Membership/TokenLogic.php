...
class CRM_Membership_TokenLogic {
  ...
  public function tokens(&$tokens) {
    ...
    foreach ($membership_types as $membership_type) {
      foreach ($base_tokens as $base_token => $base_token_name) {
        $membership_tokens["membership.{$base_token}_{$membership_type['id']}"]
          = "{$membership_type['name']} {$base_token_name}";
      }
    }
    ...
    foreach ($membership_types as $membership_type) {
      foreach ($custom_tokens as $custom_token => $custom_token_name) {
        $membership_tokens["membership.{$custom_token}_{$membership_type['id']}"]
          = "{$membership_type['name']} {$custom_token_name}";
      }
    }
    $tokens['membership'] = $membership_tokens;

    // add basic membership number tokens in the namespace called membership_number
    $number_tokens = [];
    foreach ($membership_types as $membership_type) {
      $number_tokens["membership_number.membership_number_{$membership_type['id']}"]
        = E::ts('%1 Number', [1 => $membership_type['name']]);
    }
  ...
  public function tokenValues(&$values, $cids, $job = NULL, $tokens = [], $context = NULL) {
    ...
    if (isset($tokens['membership']) && $tokens['membership'] !== []) {
      ...
      $base_tokens = $this->getBaseTokens();
      $base_tokens_used = $this->getTokensUsed($tokens['membership'], array_keys($base_tokens),
        $membership_types_used);
      $custom_tokens = $this->getCustomTokens();
      $custom_tokens_used = $this->getTokensUsed($tokens['membership'], array_keys($custom_tokens),
        $membership_types_used);

      foreach ($membership_types_used as $membership_type_id) {
        $contact_values = $this->getMembershipTokenValues($cids, $base_tokens_used, $custom_tokens_used,
          $membership_type_id);
  ...
  protected function getMembershipTokenValues($cids, $base_tokens_used, $custom_tokens_used, $membership_type_id) {
    ...
    $temp_contact2membership = CRM_Utils_SQL_TempTable::build()->setCategory('p60m_token')->getName();
          ...
          if ($field_spec !== NULL) {
            ...
            if (isset($field_spec['option_group_id']) && $field_spec['option_group_id'] !== ''
              ...
              $joins[] = "LEFT JOIN civicrm_option_value {$custom_token_used}_ov"
                ...
                . " AND {$custom_token_used}_ov.option_group_id = {$field_spec['option_group_id']}";
    ...
    foreach ($custom_token_fields as $custom_token_field_info) {
      $custom_token_name = "custom_{$custom_token_field_info['column_name']}";
      $joins[] = "LEFT JOIN {$custom_token_field_info['table_name']} AS {$custom_token_name}"
        . " ON {$custom_token_name}.entity_id = c2m.membership_id";
    ...
    while ($value_query->fetch()) {
      ...
      foreach ($base_tokens_used as $base_token) {
        $contact_values["{$base_token}_{$membership_type_id}"]
          = $this->formatTokenValue($base_token, $value_query->$base_token);
      }
      foreach ($custom_tokens_used as $custom_token) {
        $contact_values["{$custom_token}_{$membership_type_id}"]
          = $this->formatTokenValue($custom_token, $value_query->$custom_token);
      }
